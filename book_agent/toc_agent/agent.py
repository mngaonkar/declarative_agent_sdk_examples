...
async def toc_agent(state: AgentState) -> AgentState:
    context = state["context"]
    if context is None:
        logger.warning("No context provided to TOC agent.")
        raise ValueError("Context is required for TOC agent to generate response.")

    logger.info("TOC agent received context for generating response.")

    final_response = ""
    async for event in agent.invoke(context):
        if (
    ...
    logger.debug(f"TOC agent generated response: {final_response}")
    ...
    response = remove_think_content(final_response)
